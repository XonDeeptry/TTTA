...
7. **Ba lỗi chỉ lộ ra khi gọi API thật, cả ba đều lọt qua 448 test đang xanh:**
   - `pipeline.py` gọi `grade_with_fallback()` **thiếu `schema=`** mà mọi `Provider.grade()` đều bắt buộc → nhánh chấm audio CHƯA BAO GIỜ chạy được với provider thật. Nhánh pilot text lại truyền đúng. Ẩn được vì mọi test đều patch `grade_with_fallback` bằng `AsyncMock` TRẦN (nuốt mọi tham số) và chữ ký là `**grade_kwargs: Any`.
