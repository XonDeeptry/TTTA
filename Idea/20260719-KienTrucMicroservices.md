...
**Ngày:** 2026-07-19 · **Phiên bản:** 1.6 · **Trạng thái:** Đã duyệt, đang triển khai (M1 + M2 + M3 + M4 xong; **đường chấm LLM đã nghiệm thu với API key thật ngày 2026-08-25**)

**Changelog v1.6 (2026-08-25, triển khai `Idea/20260819-ChamDiemRubricV2.md` + nghiệm thu LLM thật):**

*Phần A — rubric v2 & trình soạn tiêu chí (F8–F12, xem `docs/dev-team-roles/PROGRESS.md`):*

1. **Rubric schema v2** — bổ sung quy tắc tổng hợp điểm (`sum`/`average`/`weighted_average`), quy đổi tổng → cấp độ, mô tả band dạng danh sách, lưới yếu tố con, ngân hàng nhận xét có cấu trúc, và `output_fields` (`comment`/`fix`). Mục 3.9 được viết khi CHƯA có rubric thật nào; hai file ở `Criteria-Source/` cho thấy mô hình cũ không biểu diễn nổi (KID cộng tổng 25 → 4 cấp độ; IELTS trung bình band 0–9). Có shim `normalizeRubric` v1→v2 chạy lúc ĐỌC nên **không mất dữ liệu cũ, không migration**.
2. **Hai cấu trúc chấm điểm mặc định là DỮ LIỆU, không phải hằng số** — seed vào `rubric_templates` khi bảng rỗng (khuôn `BootstrapAdminService`), có CRUD đầy đủ, sửa/nhân bản/ẩn được, có nút khôi phục bản gốc, không xóa được. Mục đích: sửa rubric không được kéo theo deploy. **Template chỉ là KHUNG — `bands` rỗng**; nội dung là phần giáo viên soạn (F12).
3. **Phân quyền theo `DashboardUser.privileges` (mảng chuỗi)**, giữ nguyên 2 role `admin`/`staff` của v1.3. Tách `rubric_template` (sửa *cấu trúc*) và `criteria_author` (soạn *nội dung*). Migration backfill `criteria_author` cho `staff` đang tồn tại để không khóa nhầm người đang dùng.
4. **Tổng điểm & cấp độ tính ở core-api**, không ở LLM và không ở worker (`lib/rubric-scoring.ts`). Báo cáo TÍNH LẠI từ `scores` + rubric đã ghim thay vì đọc cột, nên grading trước F9 không biến mất khỏi thống kê. Đồng thời **sửa bug có sẵn**: `reports.service.ts` bỏ qua `weight` khi tính trung bình.
5. **Tin nhắn ra đầu tiên có tương tác (nút bấm)** — nới ranh giới "bot không hội thoại" của v1.1 mục 1, có kiểm soát: tập hành động ĐÓNG sau tiền tố `#ilm:`, mọi text khác vẫn rơi nguyên vào luồng flag cho tư vấn. `oa.query.show` quay lại dưới dạng `user_send_text` nên không cần webhook mới/scope mới. Trường `template_type` vẫn **chờ nghiệm thu với OA thật**.
6. **Trình render prompt giờ TỒN TẠI HAI BẢN** (`criteria/prompt-render.ts` ↔ `grading/prompt.py`) — là bản trùng lặp xuyên ngôn ngữ **THỨ BA** của repo, sau `contracts.*` và `normalizeRubric`. Lý do: màn soạn tiêu chí cần xem trước prompt, mà dashboard không có bộ test nào nên một bản port ở đó sẽ không kiểm chứng được. Hợp đồng là `criteria/__fixtures__/prompt-render.fixtures.json`, được **cả jest lẫn pytest** khẳng định.

*Phần B — nghiệm thu đường chấm LLM với API key thật (2026-08-25):*

7. **Ba lỗi chỉ lộ ra khi gọi API thật, cả ba đều lọt qua 448 test đang xanh:**
   - `pipeline.py` gọi `grade_with_fallback()` **thiếu `schema=`** mà mọi `Provider.grade()` đều bắt buộc → nhánh chấm audio CHƯA BAO GIỜ chạy được với provider thật. Nhánh pilot text (nay đã gỡ, xem Phần C) lại truyền đúng — chính sự bất đối xứng đó khiến lỗi lọt qua review. Ẩn được vì mọi test đều patch `grade_with_fallback` bằng `AsyncMock` TRẦN (nuốt mọi tham số) và chữ ký là `**grade_kwargs: Any`.
   - `gemini-2.5-flash` bị Google **ngừng cấp cho người dùng mới** (404, kèm chỉ dẫn dùng `gemini-3.6-flash`). Endpoint `interactions` vẫn đúng — chỉ tên model là cũ.
   - `interaction.usage` dùng `total_input_tokens`/`total_output_tokens`/`total_thought_tokens`, KHÔNG phải `input_tokens`/`output_tokens`; `getattr(..., 0) or 0` nuốt mất khác biệt nên mọi `cost_log` ghi 0 token và `est_usd = 0` — **cảnh báo ngưỡng chi phí (mục 3.12) sẽ không bao giờ kêu**, mà không có exception hay log lỗi nào.
   > Bài học chung: mock ở ranh giới NGOÀI (`grade_with_fallback`) thì không kiểm được hợp đồng BÊN TRONG. Đã bổ sung test ràng lời gọi vào chữ ký thật của `Provider.grade` và test bám đúng hình dạng object `Usage` của SDK.
8. **Cấu hình LLM chuyển hẳn lên dashboard, không còn hardcode** — thêm `llm.gemini_model`, `llm.openai_model`, `llm.temperature`, `llm.pricing_json` vào `setting-defs.ts`. Thứ tự ưu tiên: `courses.llm_config` → settings → hằng số dự phòng. Màn Cấu hình gợi ý model bằng danh sách **lấy trực tiếp từ provider** (`GET /settings/llm-models/:provider`) nhưng vẫn cho gõ tay — một danh sách cứng sẽ lỗi thời đúng theo cách `gemini-2.5-flash` vừa lỗi thời. `llm.pricing_json` để điền đơn giá model mới mà không phải sửa code.
9. **Nhiệt độ mặc định 0.3 là một quyết định cần xem lại.** Chấm cùng MỘT clip 35 giây hai lần cho ra 4/5/5/4 (tổng 5) và 3/3/3/3 (tổng 3) — lệch 2 band trên thang 0–9. Một phần do rubric không khớp (clip thiếu nhi chấm bằng thang IELTS người lớn), nhưng **cần đo lại độ ổn định ở `llm.temperature = 0` với rubric đúng trước khi giáo viên dựa vào điểm số này**.
10. **Van chi phí (mục 3.5) đã nghiệm thu thật**: clip 19 phút bị từ chối ở mốc mặc định 7 phút TRƯỚC khi gọi LLM, kèm tin nhắn báo lại cho học viên — không tốn một đồng token nào.
11. **Lỗi prompt: nhãn phân loại của kho nhận xét lọt vào nhận xét gửi học viên.** `_render_comment_bank` in ví dụ dưới dạng `- [khen] <nội dung>` ngay dưới câu "bám theo văn phong này", nên model đọc `[khen]` như MỘT PHẦN của văn mẫu cần bắt chước. Gemini Flash bỏ qua, nhưng **Gemini Pro bám ví dụ sát hơn nên chép luôn `[khen]`/`[góp ý]` vào `comment`** — tức là chữ rác lọt tới học viên. Đã sửa bằng cách nói rõ trong chính câu dẫn rằng nhãn trong ngoặc vuông chỉ để phân loại. Vì trình render prompt là bản trùng lặp xuyên ngôn ngữ thứ ba (mục 6), sửa đồng thời `prompt.py` + `prompt-render.ts` + fixture dùng chung; cả pytest (480) lẫn jest (`src/criteria` 615) vẫn xanh. **Bài học: model mạnh hơn không chỉ chấm khác — nó còn phơi ra những chỗ prompt viết mơ hồ mà model yếu hơn vô tình bỏ qua.**
12. **So sánh Flash vs Pro trên cùng một clip (nhiệt độ 0, cùng rubric):** Flash cho tổng 4 rồi 5; Pro cho tổng 2 rồi 3. Pro **KHÔNG ổn định hơn** (vẫn lệch 1 band), tốn ~1.7× token đầu ra và chậm hơn ~30–50%. Nhưng lập luận của Pro bám rubric chặt hơn: nó nhận ra "đây là bài lặp từ đơn nên chưa thể hiện được độ trôi chảy giao tiếp" — đúng tinh thần thang IELTS — và liệt kê 8 từ phát âm sai so với 2 của Flash. **Điểm thấp hơn ở đây nhiều khả năng là ĐÚNG hơn, không phải tệ hơn**; chưa thể kết luận model nào chính xác hơn nếu chưa có bài chấm tay làm mốc đối chiếu.

*Phần C — gỡ hẳn nhánh chấm-từ-transcript (2026-08-25, quyết định chủ dự án):*

13. **Chỉ chấm TRỰC TIẾP TỪ AUDIO. Nhánh A/B chấm-từ-transcript (F2, changelog v1.5 mục MP.1) bị gỡ TOÀN BỘ** — code, test, i18n, route API, báo cáo so sánh, panel dashboard, và bảng `pilot_text_grading` (migration `20260825120000_drop_pilot_text_grading`, **phá hủy**; bảng rỗng vì cờ `limits.pilot_dual_grading` xuất xưởng ở trạng thái TẮT và chưa ai bật). Lý do nghiệp vụ: transcript không mang theo bằng chứng phát âm (`heard_as`, vị trí giây), mà `pronunciation` là tiêu chí **BẮT BUỘC** (mục 3.10) — chấm phát âm từ văn bản chỉ là phỏng đoán, nên nhánh này không bao giờ có thể làm mốc đối chiếu công bằng. Cùng lúc gỡ: `Provider.transcribe`/`grade_text` khỏi protocol và cả hai adapter, `transcribe_with_fallback`/`grade_text_with_fallback`, `build_system_instruction_text`, `ConfigStore.get_bool` (chỉ dùng cho cờ này), setting `limits.pilot_dual_grading`. **`cost_log.call_type` GIỮ NGUYÊN** (cột mở kiểu chuỗi) — các giá trị `transcription`/`text_grade` đơn giản là không còn được ghi; mọi dòng hiện có đều là `audio_grade`. Sau khi gỡ: grading-worker 443 test xanh (−37), core-api 876 test / 43 suite xanh + tsc sạch (−128), dashboard build sạch, i18n vi/en cân bằng 402=402. Nghiệm thu lại đường chấm audio thật sau khi gỡ (clip mẫu 35s, `gemini-3.6-flash`, 4285/2131 token, dừng đúng ở `awaiting_review`). **Muốn dựng lại nhánh text phải xem lại mục 3.10 trước.**
14. **`gemini-3.1-pro-preview` không dùng được cho production, dù key hợp lệ.** Cùng một key: Flash trả lời prompt một từ trong 4,5 giây; Pro-preview mất **88 giây** cho đúng prompt đó, và với payload audio thật trả 401 rồi 500 sau ~5 phút chờ mỗi lần. 401 ở đây **KHÔNG phải lỗi credential** — đây là bất ổn của tầng preview. Bài học vận hành: gặp 401/500 từ Gemini thì thử lại bằng Flash trước khi đi tìm lỗi API key. Model mặc định đã đưa về `gemini-3.6-flash` (đổi qua dashboard, không cần deploy — mục 8).
