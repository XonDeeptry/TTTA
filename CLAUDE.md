...
```bash
# zalo-gateway (services/zalo-gateway) / core-api (services/core-api) — same scripts in both
npm install          # install deps
npm test             # jest unit tests (mocked deps — no real DB/Redis/Rabbit needed)
...
# dashboard (services/dashboard) — NOT in docker-compose yet, see below
npm install
...
## Dashboard (implemented slice; full build is M4)

`services/dashboard` is a minimal React + Vite + react-i18next (vi default, en) app with exactly 3 routes: `/login`, `/settings`, `/onboarding` — pulled forward from M4 because M2's settings/onboarding APIs needed a real UI, not just curl, per the v1.3 decision. It is **not** in `docker-compose`/Caddy yet (`infra/Caddyfile` proxies `/api*` → core-api, but there's no dashboard route there) — run it via `npm run dev`, which proxies `/api` to `localhost:3001`. The other 4 subsystems (submission monitoring + review, reports/cost, criteria/prompt management) are still to build in M4, at which point the whole app gets containerized and Caddy's catch-all `handle { }` block gets a real static-file target instead of the placeholder "ilm-bot" response.

## What this repository is

A **Zalo OA homework-grading bot** for the ILM English Center, currently mid-build (Milestones 1–3 of 5 shipped — see `TASKS.md`). Students submit homework (mainly ~5-minute speaking clips) via Zalo Official Account; an LLM (Gemini Flash, chosen for direct audio input) grades against course-level criteria and replies. All planning documents are written in Vietnamese; so are code comments in `services/zalo-gateway`, `services/core-api`, and `services/grading-worker`.
